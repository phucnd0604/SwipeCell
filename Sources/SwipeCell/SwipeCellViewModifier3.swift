...
extension SwipeCellModifier {
    ...
    func getGesture() -> _EndedGesture<_ChangedGesture<DragGesture>> {
        ...
        let nonEditGragMinDistance: CGFloat = 0
        return DragGesture(
        ...
        .onChanged { value in
            ...
            handlePanGestureOnChangedEvent(width: width)
        }.onEnded { value in
            let width = value.translation.width
            handlePanGestureEndedEvent(width: width)
        }
    }

    func handlePanGestureOnChangedEvent(width: CGFloat) {
        var width = width
    ...
    func handlePanGestureEndedEvent(width: CGFloat) {
        if currentCellID != cellID {
            currentCellID = cellID
            NotificationCenter.default.post(Notification(name: .swipeCellReset, object: cellID))
        }
